...
int main(void)
{
	...
	HAL_UART_Transmit(&huart2, (uint8_t *) "Initializing LED\n\r", strlen("Initializing LED\n\r"), 300);
	 ...
	 */
	max7219_Init(3);
	HAL_UART_Transmit(&huart2, (uint8_t *) "Decode\n\r", strlen("Decode\n\r"), 300);
	max7219_Decode_Off();
	HAL_Delay(4000); //Explain this Delay
	max7219_Clean();
	HAL_UART_Transmit(&huart2, (uint8_t *) "Clean. Printing...\n\r", strlen("Clean. Printing...\n\r"), 300);
	...
	while (1)
	{
		...
		/* USER CODE BEGIN 3 */
		HAL_UART_Transmit(&huart2, (uint8_t *) "All-segment test\n\r", strlen("All-segment test\n\r"), 300);
		for(int i = 1; i <= 4*4*4; i++) {
			max7219_PrintDigit(i, NUM_8, true);
		}
		HAL_Delay(5000);

		HAL_UART_Transmit(&huart2, (uint8_t *) "All-zero test\n\r", strlen("All-zero test\n\r"), 300);
		for(int i = 1; i <= 4*4*4; i++) {
			max7219_PrintDigit(i, counter, false);
		}
		HAL_Delay(1000);
